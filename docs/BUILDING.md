...
| 来源 | 内容 | 用途 |
| --- | --- | --- |
| [`RELEASE_NOTES.md`](../RELEASE_NOTES.md) | 每次发版都变的更新说明 | 老用户升级后看到；同时是 GitHub Release 正文 |
| [`FEATURES.md`](../FEATURES.md) | 几乎不变的全功能总览（幽默风） | **只在用户第一次接触工具时**展示一次（安装器首装 / 游戏内首启） |
...
```powershell
# ⓪ 生成公告与功能总览源码（RELEASE_NOTES.md / FEATURES.md -> 面板 / 安装器）
