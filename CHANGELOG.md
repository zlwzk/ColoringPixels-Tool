...
- **「自动检测」写死了只扫《Coloring Pixels》**：在《涂色大师》标签下点检测，永远找不到本机明明
  装着的游戏 —— 这就是「路径识别不对」的根源，用户只能每次手动浏览选目录。现在一次探测两款、
  按当前标签取用，且**哪款没找到就深扫哪款**（原来是「两款都找不到才深扫」，所以装了 CP 就再也顾不上 PCS）。
- **安装器不记游戏目录**：`_dirByGame` 只是内存字典，关掉即丢。现在落盘到
  `%APPDATA%\ColoringPixelsTool\installer.settings`（原子写入），下次打开直接可用；
  只有目录被移动 / 游戏被卸载后才退回自动检测。
- **深扫只认 Steam 标准目录名**：装在 `D:\Games\PixelCrossStitch` 这类自定义目录里就扫不到。
  现在目录名「沾边」（PixelCrossStitch / 涂色大师 / 像素梦想家……）也会看一眼，
  最终仍由 exe 与数据目录名二次确认，不会误判。

### 修复与优化（《涂色大师：像素梦想家》独立助手）

- **调色板校准可以记住了**：以前每次打开助手都得重新 <kbd>F5</kbd> 框一遍调色板，
  不框就完全没法开始自动绘图 —— 这是「自动绘图用不起来」最常见的卡点。
  现在存进 `%APPDATA%\PixelAssist\Assist.palette`（列 / 行 / 每个色块的位置与颜色全都存），打开即用。
- **新增「识别调色板」**：照着存档里这一关的目标颜色去屏幕上反查色块位置，不必手动框选。
  比原来的「跳变峰投影」稳得多 —— 色块颜色相近、边框很细、画面有渐变阴影时，投影法常把行列数错，
  自动绘图就会莫名涂错颜色；颜色反查没把握时自动退回投影法。
- **点「开始」不再静默失败**：缺画布 / 调色板 / 关卡时弹窗写清楚差什么、按哪个键，
  并给出「只涂当前颜色」这条完全不依赖调色板识别的路径。
- **自动绘图页重排**：① 选择关卡 → 状态卡与**准备情况**（画布 / 调色板 / 关卡，缺哪项写哪项）
  → ② 标定画布与调色板 → ③ 开始。以前只有一句「调色板未校准」，用户根本不知道还差什么。
- **记住窗口位置、上次停留的页签与上次在涂的关卡**：下次打开直接回到原处；
  窗口落点若已不在任何屏幕内则不套用（换显示器之后不会把窗口扔到看不见的地方）。
...
[2.3.2]: https://github.com/zlwzk/ColoringPixels-Tool/releases/tag/v2.3.2
