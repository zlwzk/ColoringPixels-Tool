...
前者是带图形面板的 BepInEx 插件（进入关卡后按 <kbd>F1</kbd> 打开）；后者的游戏本体是 IL2CPP，无法注入，改用独立助手 `PixelAssist.exe`
（刷新存档后按 <kbd>F5</kbd> 框选调色板、<kbd>F7</kbd> 框选画布、<kbd>F6</kbd> 开始自动绘图，也保留逐行扫的人工辅助）。
两款游戏共用同一份等级与进度存档。
...
| 《涂色大师：像素梦想家》 | **3071670** | 独立助手 `PixelAssist.exe`（IL2CPP，无法注入插件；自动绘图 / 人工辅助 / 预览 / 等级） |
...
| **独立助手** `PixelAssist.exe` | 《涂色大师：像素梦想家》的辅助工具：读存档里每格的目标色自动绘图，或退回「人工辅助」逐行匀速扫过（每一格仍由游戏自己判定），另有预览与等级页。 |
...
- **助手自动绘图**：读《涂色大师》存档里每格的目标色，按颜色分组自动点击 / 拖拽填涂；调色板自动识别，另有慢 / 中 / 快三档速度预设、「按存档推算格子」与整幅图案预览。
- **等级与进度两游戏共用**：插件与助手读写同一份 `%APPDATA%\ColoringPixelsTool` 存档，两条经验线、等级、称号与统计双向合并取最大，重装或换游戏都不会丢进度。
...
助手面板分「首页 / 自动绘图 / 人工辅助 / 预览 / 等级 / 设置」六页：首页有关卡卡片与计时器，
人工辅助页另有「按存档推算格子」「区域微调（四角坐标 + 弯边）」「快速模板」，
设置页可查看更新公告、功能总览并一键提交 Bug 反馈。
...
- **设置**：用户资料与背景、面板外观、悬浮 HUD、快捷键、游戏汉化、推荐预设、自动化与安全、语音交互、Bug 反馈、新手指引、功能总览（可随时重看全部功能）。
...
```
%APPDATA%\PixelAssist\        （Assist.region / Assist.settings / Assist.ui / Presets\*.txt）
```
...
3. 「自动绘图」页点「刷新存档」，读出这一关每格的目标色；
4. 按 <kbd>F5</kbd> 框选游戏里的调色板，点「识别调色板」；
5. 按 <kbd>F7</kbd> 框选整个画布（或到「人工辅助」页点「按存档推算格子」自动算出行数与步长），再按 <kbd>F6</kbd> 开始；
6. <kbd>F8</kbd> 随时急停，<kbd>F12</kbd> 开关遮罩覆盖层。

**人工辅助**：不读存档也可以，退回「人工辅助」页按老办法逐行扫过；是否涂对仍由游戏自己判定，
所以进度和统计与你手涂一致。

助手与插件共用同一份等级 / 进度存档：在《Coloring Pixels》攒的等级，打开助手一样能看到。
