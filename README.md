...
# Coloring Pixels Tool

**《Coloring Pixels》的非官方辅助工具 —— 一键安装，开箱即用**

自动定位游戏目录、部署文件、启动游戏；进入任意关卡后按 <kbd>F1</kbd> 打开游戏内面板。
...
支持的游戏：《Coloring Pixels》（Steam AppID **897330**，BepInEx 插件 + 游戏内面板）。

工具由两部分组成：

| 组件 | 说明 |
| --- | --- |
| **安装器** `ColoringPixelsTool-Setup.exe` | 单文件、免安装、可视化。自动检测游戏目录 → 部署文件 → 启动游戏，并支持检查更新。 |
| **插件** `ColoringPixelsTool.dll` | 游戏内面板：分「自动完成」「人工辅助」两大模块，提供涂色、拟人涂色、定时连图、内容解锁、悬浮 HUD 等能力。 |
...
- **人工辅助**：不代替你点，而是「屏幕扫描 + 模拟鼠标」逐行匀速扫过你框选的区域，每一格仍由游戏自己判定是否涂对，进度与统计和手涂一致。
...
- **体验增强**：语音换色、等级与称号、经验与统计、游戏内「推荐预设」按钮、新手指引、爱心重置二次确认。
  首次安装 / 首次启动会给一份完整的**功能总览**（幽默风，讲清所有功能），之后更新只弹当版更新公告，不再重复糊一遍。等级系统有**两条互不相干的经验线**——「人工辅助」（手动点击拖涂、扫描引擎、在线时长）与「自动绘图」（一键涂完、挂机、自动切图），各自升级、各有各的称号，只有对应模块的成果才增长对应那条。经验节奏刻意放得很慢（在线挂机 1 XP / 30 秒、自动涂色 1 XP / 150 格），越往后每级越久，等级只升不降。
...
- 已安装 Steam 版的《Coloring Pixels》，且需 **32 位版本**：本插件不支持 64 位主程序
...
4. 确认游戏目录后点击「**一键安装**」，等待进度条走完。
5. 安装器会**自动启动游戏**（Steam 版会交给 Steam 拉起，避免直启 exe 闪退）：进入任意关卡后按 <kbd>F1</kbd> 打开面板。
...
卸载只会移除工具文件与（可选的）BepInEx 本体，不会触碰游戏存档与等级进度。
...
- **自动化**：设定总时长、换图间隔、连续涂图与涂色速度（慢 / 中 / 快三档预设，或**自己拉手速、笔长、停笔概率**），挂机连涂多张图；自动化期间的速度只属于它自己，结束后自动还给「拟人」页签。
- **辅助**：画布颜色高亮（预设色块 / R·G·B、填充·描边·四角框、不透明度、只高亮未涂格子、呼吸闪烁）。
- **解锁**：解锁全部 DLC、免费提示（普通 / 重提示）、显示隐藏书籍、标记书籍完成、解锁 Steam 成就。
- **设置**：用户资料与背景、面板外观、悬浮 HUD、快捷键、推荐预设、自动化与安全、语音交互、Bug 反馈、新手指引、功能总览（可随时重看全部功能）。
...
可直接编辑该文件修改热键、参数与默认值（面板内的改动也会实时写回）。主要分组：`0-通用`、`1-解锁`、`2-显示`、`3-拟人涂色`、`4-人工辅助`、`5-自动化`、`6-面板`、`8-推荐预设`、`9-快捷键`、`A-功能开关`、`B-反馈`。

人工辅助的区域与参数存在 `BepInEx\config\ColoringPixelsTool.Assist\`。
...
ColoringPixelsTool-Setup.exe [选项]
  ...
  --game=cp           指定目标游戏（本工具只有一款，纯兼容旧快捷方式）
  --silent            静默安装，不显示界面
...
- 请选择包含游戏主程序 `ColoringPixels.exe` 的那一层目录。
- 或使用 `--deep` 参数进行全盘深度扫描。
- 64 位主程序不受支持：本插件基于 BepInEx 5（x86），仅支持 32 位版本。
...
│  ├─ Assist*.cs                人工辅助引擎（区域模型 / 屏幕扫描 / 模拟鼠标）
│  └─ AssistAutoDetect.cs       一键识别画布区域与格子大小（读游戏内部画布几何）
├─ installer/                   安装器源码（C# 5 / WinForms，单文件）
│  └─ payload/                  部署包覆盖层（定制 doorstop_config.ini 等）
├─ scripts/                     构建脚本（PowerShell）
│  ├─ build-changelog.ps1       由 RELEASE_NOTES.md / FEATURES.md 生成公告与功能总览源码
│  ├─ build-mod.ps1             编译插件
...
├─ RELEASE_NOTES.md             更新公告（唯一来源，同时是 Release 正文）
├─ FEATURES.md                  全功能总览（首次安装 / 首次启动时展示一次）
├─ VERSION                      版本号（唯一来源）
...
- 本项目与《Coloring Pixels》的开发商、发行商及 Steam 无关，游戏相关的全部权利归其所有者所有。
- 请支持正版游戏。
